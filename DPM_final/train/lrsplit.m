...
nrfts=model.features.dim-model.features.hog;
% Get warpped positives
[warped,warpedD] = warppos(model, pos); %dld kanei imresize tis eikones gia na xwrane stis diastaseis toy monteloy

% Cache features
fprintf('Caching features\n');
numpos = length(warped);
feats=features2(single(warpedD{1,1}),double(warped{1,1}), model.sbin,1,model.filters.size,nrfts,interval_endpoints);   % greg, 6/2: modified here based on the new feats demands
fi = model.symbols(model.rules{model.start}.rhs).filter;
fsize = model.filters(fi).size;
dim = numel(feats(1:fsize(1),1:fsize(2),:));
F = zeros(numpos, dim);
parfor i = 1:numpos
  tic_toc_print('%s %s: lrsplit features: %d/%d\n', ...
                procid(), model.class, i, length(warped));
  feats=features2(single(warpedD{i,1}),double(warped{i,1}), model.sbin,1,model.filters.size,nrfts,interval_endpoints);
  F(i,:) = reshape(feats, [1 dim]);
end
