...
%   randneg   1 => use random negatives
...
if ~cont
  ...
  [max_dim, max_nbls] = max_fv_dim(model); % max_dim: max dimension of that feature vector over all derivations,
                                            %max_nbls: max number of blocks used
...
fv_cache('set_model', blocks, lb, rm, lm, cmps, C, true); 
                    %args: lb=lower bounds as cell array of vectors, rm=regmult as 1d vector,...
                    % cmps=componentblocks as cell array of vectors
...
for t = 1:iter                                     
  ...
  if ~cont || t > 1
    % compute hinge loss (synartisi L) on foreground examples before relabeling
    if warp == 0
...
% of the form Q -> F.
numpos = length(pos);
interval_endpoints=model.features.displacement.interval_endpoints;
[warped,warpedD] = warppos(model, pos); 
...
for i = 1:numpos
  ...
  % get example
  im = warpedD{i,1}; imrgbd=warped{i,1};
  feat = features2(single(im),double(imrgbd), model.sbin,1,model.filters(fi).size,model.features.dim-model.features.hog,interval_endpoints); 
  key = [i 0 0 0];
  bls = [obl; fbl] - 1;
...
for i = 1:numneg
  ...
  im = imreadx(neg(i));
  ftrgbd=features_64(double(im),model.sbin);  % hog 64 features
  imd=im(:,:,4);out=floor(size(imd)/model.sbin); im2=im2blocks(single(imd),model.sbin,out(1),out(2));
  ftdisp=make_feats_idea7(im2,interval_endpoints,rsize(1),rsize(2),model.features.dim-model.features.hog,size(interval_endpoints,2)-2);
  if size(ftrgbd,2) > rsize(2) && size(ftrgbd,1) > rsize(1)
    for j = 1:rndneg
      x = random('unid', size(ftrgbd,2)-rsize(2)+1); % create random numbers with uniform distribution
      y = random('unid', size(ftrgbd,1)-rsize(1)+1);
      miny=min(y*rsize(1)+1,size(ftdisp,1)-rsize(1));       % ensure that the selected displacement features are inside the boundaries
      minx=min(x*rsize(2)+1,size(ftdisp,2)-rsize(2)); 
      ftdisp_rsize=ftdisp(miny:miny+rsize(1)-1,minx:minx+rsize(2)-1,:);
      fthog_rsize=ftrgbd(y:y+rsize(1)-1, x:x+rsize(2)-1,:);
      f=cat(3,fthog_rsize,ftdisp_rsize);
