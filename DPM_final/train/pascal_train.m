...
data_augmentation = false;  % make this true, in case you want to use the augmented data
...
if data_augmentation 
    [posRen,pos, impos2] = add_rendered(cls, pos, impos,neg(1,end).dataid );
end
...
try
  ...
  seed_rand();
  for i = 1:n
    models{i} = root_model(cls, spos{i}, note); %ftiaxnei kai epistrefei ena model struct poy exei ena root kai ena non-terminal 
    %symvolo kai ena kanona (rule)
    %models{i}.!parameters=models{i}.features.displacement.interval_endpoints;
    % Split the i-th aspect ratio group into two clusters: 
    % left vs. right facing instances
    inds = lrsplit(models{i}, spos{i});
    if data_augmentation
        spos_init=spos{i}(inds);
        sposR = spos_augmentation(spos_init,posRen);
    else
        sposR = spos{i}(inds);
    end
    % Train asymmetric root filter on one of these groups
    models{i} = train(models{i}, sposR, neg_large, true, true, 1, ... % model, pos, neg, warp, randneg, iter
                      1, max_num_examples, fg_overlap,  ...  % negiter, max_num_examples, fg_overlap,
                      0, false, ['lrsplit1_' num2str(i)]);   % num_fp, cont, tag, C
  end
...
try
  ...
  for i = 1:n
    ...
    models{i} = train(models{i}, sposR, neg_small, false, false, 4, ... 
                      3, max_num_examples, fg_overlap, ...
                      0, false, ['lrsplit2_' num2str(i)]);
  end
  save([cachedir cls '_lrsplit2'], 'models');
end

% Train a mixture model composed of all aspect ratio groups and 
...
function sposR = spos_augmentation(spos_init,posRen)
    cnt_new=0;
    for cnt=1:size(spos_init,2)
        cnt_new=cnt_new+1;
        sposR(1,cnt_new)=spos_init(1,cnt);
        if ~isempty(spos_init(1,cnt).render.im)
            renderedim=spos_init(1,cnt).render.im;
            for sz=1:max(size(renderedim))
                cnt_new=cnt_new+1;
                sposR(1,cnt_new)=posRen(1,renderedim(sz));
            end
        end
    end
