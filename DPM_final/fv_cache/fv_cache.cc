...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  { // Handle input command
    ...
    for (int i = 0; handlers[i].func != NULL; i++)
      if (handlers[i].cmd.compare(cmd) == 0)//greg, vriskei dld poia entoli exei dothei sygrinontas tis diathesimes epiloges (proigoymeni synartisi)
        handlers[i].func(nlhs, plhs, nrhs, prhs); //greg, afoy taytisei entoli me arithmo, paei se struct handler_registry (orizetai ~30 grammi) kai kalei tin antistoixei synartisi *func 
    mxFree(cmd);
  }
