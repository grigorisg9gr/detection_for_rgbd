...
L = model_sort(model); % taksinomei ta nonterminal symbols toy monteloy me topologiki diataksi apo riza
for s = L
  for r = model.rules{s}
...
function model = filter_dims(model) %greg, temp: tin prwti fora dimioyrgei ta pedia gia max_dim
