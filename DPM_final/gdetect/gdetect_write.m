    ...
    if model.symbols(sym).type == 'T'
      ...
      if write_block(bl)
        ex = addfilterfeat(model, ex,             ...
                           ...
                           pyra.padx, pyra.pady,  ...
                           t(j).ds, fi,           ...
                           pyra.feat{t(j).l,1},pyra.feat{t(j).l,2});
      end
    else
...
function ex = addfilterfeat(model, ex, x, y, padx, pady, ds, fi, featHOG,featDISPF)
% model object model
...
% fi    filter index
% featHOG  padded HOG feature map
% featDISPF padded DISP image in blocks

fsz = model.filters(fi).size;
% remove virtual padding
fy = y - pady*(2^ds-1);
fx = x - padx*(2^ds-1);
f1 = featHOG(fy:fy+fsz(1)-1, fx:fx+fsz(2)-1, :); % hog part
f2=extract_displacement_feats_filter_size(featDISPF(fy:fy+fsz(1)-1, fx:fx+fsz(2)-1, :),fsz,single(model.features.displacement.interval_endpoints),model.features.dim-model.features.hog);
f=cat(3,f1,f2);
