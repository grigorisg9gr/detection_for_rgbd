...
for level = 1:pyra.num_levels
  score = model.symbols(model.start).score{level}; %greg, 27/1 (com): this is the overlap with the groundtruth box (computed in apply_L_output (gdetect_pos))
...
[ds, bs, trees] = get_detection_trees(model, pyra.padx, pyra.pady, ...
                                      pyra.scales(:,1), X, Y, L, S, get_loss);
