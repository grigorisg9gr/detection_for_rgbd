...
%HOME_DIR='/home/user/';
BASE_DIR    = 'directory_of_dpm_files';

%DB_BASE_DIR = HOME_DIR;
DB_BASE_DIR = 'directory_of_database_files';

PASCAL_YEAR = '2007'; 	% PASCAL dataset year to use
DATASET='nyu';          % name of the dataset (should be the name of the main folder where images, annotations are)
SPLIT = 'split1'; 
...
conf = cv(conf, 'dataset', DATASET);
conf = cv(conf, 'split', SPLIT);

% Parent directory that everything (model cache, VOCdevkit) is under
conf = cv(conf, 'paths.base_dir', BASE_DIR);
conf = cv(conf, 'paths.db_base_dir', [DB_BASE_DIR '/' conf.dataset '/']); 
conf = cv(conf, 'paths.db_annotation_dir', [conf.paths.db_base_dir 'ImageSets/Main_' conf.split '/']); %greg, 18/11: to use default annotation of the Database we use
% Path to this file
...
conf = cv(conf, 'paths.model_dir', [conf.paths.base_dir '/' ...
                                    conf.project '/' conf.dataset '/' conf.split '/']);
...
conf = cv(conf, 'features.hog',64);                         % HOG RGB+D feat size
conf = cv(conf, 'features.displacement.N',7);               % intervals (positive), total number is 2*N+1 (positive, negatives,zero-one)
conf = cv(conf, 'features.displacement.upperL',2^16);       % upper limit of values for depth images 
conf = cv(conf, 'features.displacement.size',conf.features.displacement.N*2+1);

a = nthroot(conf.features.displacement.upperL, conf.features.displacement.N+1);
interval_endpoints = [0 a.^[1:conf.features.displacement.N+1]];
totalFts = conf.features.displacement.size +1 + conf.features.hog; % the 1 is for the truncation dimension
%totalFts=floor((conf.features.displacement.N*2+1)/4+1)*4+conf.features.hog;    % greg, 14/3: This is for the fconvsee, if changed here, change the c file as well!!!!!!!!!!!!!!!!
conf = cv(conf, 'features.displacement.interval_endpoints',single(interval_endpoints));
conf = cv(conf, 'features.dim', totalFts); 
conf = cv(conf, 'features.truncation_dim', totalFts);
