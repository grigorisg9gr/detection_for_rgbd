...
[gtids,t]=textread(sprintf('%s%s%s%s',VOCopts,file_loc, testset,'.txt'),'%s %d');
    for i=1:length(gtids)
        ...
        recs(i)=PASreadrecord(sprintf('%s%s%s%s%s%s',conf.paths.db_base_dir,'Annotations/',cls,'/', gtids{i}, '.xml'));
    end
%     save(cp,'gtids','recs');
% end
