...
double uu[9] = {1.0000,    //ektos apo to 1o stoixeio toy dianysmatos ta ypoloipa einai antisymmetrika
		0.9397, 
		0.7660, 
		0.500, 
		0.1736, 
		...
		-0.7660, 
		-0.9397};
double vv[9] = {0.0000,   //ektos apo to 1o stoixeio toy dianysmatos ta ypoloipa einai symmetrika
		0.3420, 
		0.6428, 
		0.8660, 
		0.9848, 
...
mxArray *processD(const mxArray *mximage, const mxArray *mxsbin) {
  double *im = (double *)mxGetPr(mximage);
  const int *dims = mxGetDimensions(mximage);
//mexPrintf("mxGetNumberOfDimensions(mximage)=%d\tdims[2]=%d\t\n",mxGetNumberOfDimensions(mximage),dims[2]);
  if (mxGetNumberOfDimensions(mximage) != 2 ||
      mxGetClassID(mximage) != mxDOUBLE_CLASS)
    mexErrMsgTxt("Invalid input, here is depth");
