function pyra = featpyramid_xren(im, model)
...
for i = 1:interval
  ...
  scaled2 = imresize(imresize(im,2), 1/sc^(i-1));
  % "first" 2x interval
  pyra.feat{i} = single(features_xren(scaled2, sbin));
  pyra.scale(i) = 2/sc^(i-1);
  % "second" 2x interval
  pyra.feat{i+interval} = single(features_xren(scaled, sbin));
  ...
  for j = i+interval:interval:max_scale
    ...
    pyra.feat{j+interval} = single(features_xren(scaled, sbin));
