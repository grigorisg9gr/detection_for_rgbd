...
//
// this version puts pos/neg coefficients in separate bins  (3x feature dimension)
//
...
void smooth_bilinear(MatrixXf& hist, MatrixXf& codes, int hh, int ww, int ndic, int nchannel, int blocksize)
{
  int ny=floor(hh/blocksize);
  int nx=floor(ww/blocksize);

  if (hist.rows()!=ny*nx || hist.cols()!=ndic*nchannel)
    hist.resize(ny*nx,ndic*nchannel);
  ...
  int sparsity=codes.cols()/2/nchannel;
  ...
  for(int s=0; s<sparsity; s++)
  {
    ...
    for(int y=ystart; y<=yend; y++)
      ...
      {
        int p=y+x*hh;
        hist( iyp(y,x)+ixp(y,x)*ny, (int)codes(p,2*s)+ichannel*ndic ) += vx1(y,x)*vy1(y,x)*codes(p,2*s+1+ichannel*2*sparsity);
        hist( iyp(y,x)+1+ixp(y,x)*ny, (int)codes(p,2*s)+ichannel*ndic ) += vx1(y,x)*vy0(y,x)*codes(p,2*s+1+ichannel*2*sparsity);
        hist( iyp(y,x)+(ixp(y,x)+1)*ny, (int)codes(p,2*s)+ichannel*ndic ) += vx0(y,x)*vy1(y,x)*codes(p,2*s+1+ichannel*2*sparsity);
        hist( iyp(y,x)+1+(ixp(y,x)+1)*ny, (int)codes(p,2*s)+ichannel*ndic ) += vx0(y,x)*vy0(y,x)*codes(p,2*s+1+ichannel*2*sparsity);
      }
  }
  }
  return;
...
void mexFunction( int nlhs, mxArray *plhs[] , int nrhs, const mxArray *prhs[] )
{
    ...
    MatrixXf omp_codes_i(im_w*im_h,sparsity*2);

    omp( DtX, G, sparsity, omp_codes_i );

    int nchannel=3;
    MatrixXf omp_codes(im_w*im_h,sparsity*2*nchannel);

    // half-way rectification
    for(int k=0; k<sparsity; k++) {
      omp_codes.col(2*k)=omp_codes_i.col(2*k);
      omp_codes.col(2*k+1) = omp_codes_i.col(2*k+1).array() * (omp_codes_i.col(2*k+1).array()>=0).cast<float>();
      omp_codes.col(2*k+2*sparsity)=omp_codes_i.col(2*k);
      omp_codes.col(2*k+1+2*sparsity) = -omp_codes_i.col(2*k+1).array() * (omp_codes_i.col(2*k+1).array()<0).cast<float>();
      omp_codes.col(2*k+4*sparsity)=omp_codes_i.col(2*k);
      omp_codes.col(2*k+1+4*sparsity) = omp_codes_i.col(2*k+1).array().abs();
    } 

    int ny=(int)(im_h/sbin);
    int nx=(int)(im_w/sbin);

    MatrixXf hist(ny*nx,ndic*nchannel);

    smooth_bilinear(hist, omp_codes, im_h, im_w, ndic, nchannel, blocksize);
